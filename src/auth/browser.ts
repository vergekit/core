...
export interface AuthBrowserOptions<
  TConfig extends Pick<AnyDefinedAuthConfig, 'browser'> = AnyDefinedAuthConfig,
> {
  authConfig?: TConfig;
...
function getDefaultErrorMessage(
  options: Pick<
    AuthBrowserOptions,
    'authConfig' | 'defaultErrorMessage'
  > = {},
) {
  return (
    options.defaultErrorMessage ??
    options.authConfig?.browser.defaultErrorMessage ??
...
export function extractAuthErrorMessage(
  payload: unknown,
  options: Pick<AuthBrowserOptions, 'authConfig' | 'defaultErrorMessage'> = {},
) {
  const defaultErrorMessage = getDefaultErrorMessage(options);
...
function getFallbackRedirect(
  form: HTMLFormElement,
  options: Pick<AuthBrowserOptions, 'defaultSuccessRedirect'>,
) {
  const callbackURL = new FormData(form).get('callbackURL');
  return (
    resolveAuthRedirectTarget(callbackURL) ??
    options.defaultSuccessRedirect ??
    '/'
  );
}
...
function getSuccessRedirect(
  ...
  return (
    ...
    getFallbackRedirect(form, options)
  );
}
...
export async function handleAuthFormSubmit(
  ...
  try {
    ...
    if (!response.ok) {
      setFormError(form, extractAuthErrorMessage(payload, options));
      return null;
    }

    return getSuccessRedirect(payload, form, options);
  } catch {
    setFormError(form, getDefaultErrorMessage(options));
    return null;
  } finally {
...
export function mountAuthForms(
  ...
  for (const form of forms) {
    form.addEventListener('submit', async (event) => {
      ...
      const redirectTo = await handleAuthFormSubmit(form, options);
